...
func UpdateSystemConfig(c *fiber.Ctx) error {
	...
	conf := models.SystemConfig{Key: req.Key, Value: req.Value}
	database.DB.Save(&conf) // Create or Update
	return c.JSON(conf)
}

func GetAdminPaymentMethods(c *fiber.Ctx) error {
	var methods []models.PaymentMethod
	database.DB.Order("type asc").Find(&methods)
	return c.JSON(methods)
}

func TogglePaymentMethod(c *fiber.Ctx) error {
	id := c.Params("id")
	var method models.PaymentMethod
	if err := database.DB.First(&method, "id = ?", id).Error; err != nil {
		return c.Status(404).JSON(fiber.Map{"message": "Method not found"})
	}
