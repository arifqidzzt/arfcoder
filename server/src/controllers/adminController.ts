...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    const salesData = await prisma.order.aggregate({ _sum: { totalAmount: true }, where: { status: 'PAID' } });
    res.json({ totalOrders, totalProducts, totalUsers, totalSales: salesData._sum.totalAmount || 0 });
...
export const updateDeliveryInfo = async (req: Request, res: Response) => {
  try {
    ...
    const order = await prisma.order.update({ where: { id: id as string }, data: { deliveryInfo, status: 'SHIPPED' } });
    res.json(order);
  } catch (error) { res.status(500).json({ message: 'Error', error }); }
};
...
export const getUserChatHistory = async (req: Request, res: Response) => {
  try {
    ...
    const messages = await prisma.message.findMany({
      where: { OR: [{ senderId: userId }, { senderId: req.user?.userId, isAdmin: true }] },
