  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        toast.error(t('auth.verify_desc'));
      ...
      <main className="flex-grow flex items-center justify-center px-4 py-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white/50 backdrop-blur-xl border border-white/20 p-8 rounded-3xl shadow-xl">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold tracking-tight mb-2">{t('auth.login_title')}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? '...' : t('auth.btn_login')} <ArrowRight size={18} />
            </button>
          </form>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase tracking-wider font-bold text-gray-400">
              <span className="px-2 bg-white/0 backdrop-blur-xl">Or Continue With</span>
            </div>
          </div>
          ...
          <p className="mt-8 text-center text-sm text-gray-500">
            {t('auth.no_account')} <Link href="/register" className="text-accent font-bold hover:underline">{t('auth.register_link')}</Link>
          </p>
        </div>
      </main>
    </div>
