...
import { ShieldCheck, Truck, CreditCard, ArrowRight, MapPin, X } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  // Core API Payment Selector States
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Check if Core API mode with payment methods
  ...
  if (showPaymentSelector) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <Navbar />
        <main className="max-w-3xl mx-auto px-4 py-24 w-full">
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100">
              ...
              {Object.entries(GROUPED_PAYMENT_METHODS).map(([groupName, methods]) => {
                ...
                return (
                  <div key={groupName} className="border-b border-gray-100 pb-4 last:border-0">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{groupName}</h3>
                    <div className="space-y-2">
                      {availableMethods.map(method => (
                        <label
                          key={method.id}
                          className={`flex items-center gap-3 p-4 border-2 rounded-2xl cursor-pointer transition-all ${selectedPaymentMethod === method.id
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 bg-black text-white rounded-2xl font-bold hover:bg-gray-800 transition-all disabled:bg-gray-300 flex items-center justify-center gap-2"
              >
                {loading ? 'Memproses...' : 'Lanjutkan'} <ArrowRight size={18} />
              </button>
            </div>
          </div>
        </main>
