...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    { href: '/admin', icon: <BarChart3 size={20} />, label: t('admin.dashboard') },
    { href: '/admin/products', icon: <Package size={20} />, label: t('navbar.products') },
    { href: '/admin/orders', icon: <ShoppingBag size={20} />, label: t('navbar.orders') },
    { href: '/admin/users', icon: <Users size={20} />, label: t('admin.users') },
    { href: '/admin/vouchers', icon: <Ticket size={20} />, label: t('admin.vouchers') },
    { href: '/admin/flash-sale', icon: <Zap size={20} />, label: 'Flash Sale' },
    { href: '/admin/services', icon: <Layers size={20} />, label: t('navbar.services') },
    { href: '/admin/payments', icon: <CreditCard size={20} />, label: t('admin.payments') },
          ...
          <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
              ...
              <Link href="/" target="_blank" className="flex items-center space-x-3 px-4 py-3 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                ...
                <span className="font-medium">{t('admin.view_site')}</span>
              </Link>
            </nav>
